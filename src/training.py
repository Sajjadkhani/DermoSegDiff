...
logger = get_logger(
    filename=f"{config['model']['name']}", dir=f"logs/{config['dataset']['name']}")
print_config(config, logger)

# create the writer for tensorboard
writer = SummaryWriter(
    f'{config["run"]["writer_dir"]}/{config["model"]["name"]}')
...
try:
    ...
    logger.exception(
        "You need to determine the EMA parameters at <config.training>!")
...
if config["run"]["continue_training"] or config["training"]["intial_weights"]["use"]:
    ...
    try:
        ...
        if config["run"]["continue_training"]:
            if checkpoint["epochs"] > checkpoint["epoch"] + 1:
                ...
                logger.info(
                    f"Loaded the model state (ep:{checkpoint['epoch']+1}/{checkpoint['epochs']}) to continue training from the following path:")
        ...
        logger.warning(
            "There is a problem with loading the previous model to continue training.")
        logger.warning(
            " --> Do you want to train the model from the beginning? (y/N):")
        user_decision = input()
        if (user_decision != "y"):
            exit()
else:
    ...
    if os.path.isfile(model_path):
        logger.warning(
            f"There is a model weights at determind directory with desired name: {ID}")
        logger.warning(
            " --> Do you want to train the model from the beginning? It will overwrite the current weights! (y/N):")
        user_decision = input()
        if (user_decision != "y"):
            exit()
...
for epoch in range(start_epoch, epochs):
    ...
    tr_losses_dict = dict(
        (n, np.mean([d[1][n] for d in tr_losses])) for n in lns)

    # ---------- vl losses -------------
    assert lns == vl_losses[0][1].keys(
    ), "Ops... reported losses are different between tr and vl!"
    vl_losses_dict = dict(
        (n, np.mean([d[1][n] for d in vl_losses])) for n in lns)
